...
void Player::_LoadArenaTeamInfo()
{
    ...
    for (auto const& itr : ArenaTeam::ArenaSlotByType)
        if (uint32 arenaTeamId = sCharacterCache->GetCharacterArenaTeamIdByGuid(GetGUID(), itr.second))
        {
            ...
            if (!arenaTeam)
            {
                LOG_DEBUG("bg.arena", "Player::_LoadArenaTeamInfo: Team with ID {} not found.", arenaTeamId);
                continue;
            }
            ArenaTeamMember const* member = arenaTeam->GetMember(GetGUID());
            if (!member)
            {
                LOG_DEBUG("bg.arena", "Player::_LoadArenaTeamInfo: No members in the arena team ({}) was found.", arenaTeamId);
                continue;
            }
    ...
    if (result)
    {
        do
        {
            ...
            else
            {
                ...
                LOG_DEBUG("entities.player", "ActionButton loading problem, will be deleted from db. player: {}, guid: {}, button: {}, action: {}, type: {}", GetName(), GetGUID().GetCounter(), button, action, type);
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (Item* item = _LoadItem(trans, zoneId, timeDiff, fields))
            {
                ...
                {
                    LOG_DEBUG("entities.player", "Player::_LoadInventory: player ({}, name: '{}') has item ({}, entry: {}) which can't be loaded into inventory (Bag GUID: {}, slot: {}) by reason {}. Item will be sent by mail.",
